...
app = Flask(__name__)
app.config.update(MAX_TRAINS=10,
                  MAX_MINUTES=30,
                  CACHE_SECONDS=60,
                  THREADED=True)
...
def response_wrapper(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not isinstance(resp, Response):
            ...
            resp = Response(response=json.dumps(resp, cls=CustomJSONEncoder),
                            status=200,
...
def add_cors_header(resp):
    if app.config['DEBUG']:
        ...
        resp.headers['Access-Control-Allow-Origin'] = app.config[
            'CROSS_ORIGIN']
...
def by_route(route):
    ...
    if route.islower():
        return redirect(request.host_url + 'by-route/' + route.upper(),
                        code=301)
    ...
    try:
        ...
        resp = Response(response=json.dumps({'error': 'Station not found'}),
                        status=404,
                        mimetype="application/json")
